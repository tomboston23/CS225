...
void LPHashTable<K, V>::remove(K const& key)
{
    ...
    if(table[idx] == NULL) return;//nothing to remove
    delete table[idx];
    table[idx] = NULL;
    elems--;
}
...
void LPHashTable<K, V>::resizeTable()
{
    ...
    std::pair<K, V>** table2 = new std::pair<K, V>*[newsize];
    bool* should_probe2 = new bool[newsize]; 
    ...
    for (unsigned i = 0; i < size; ++i) {
        if (table[i] != NULL) {
            unsigned hash = hashes::hash(table[i]->first, newsize);
            for (unsigned j = 0; i < newsize; ++j) {
                int new_idx = (hash+j)%newsize;
                if (table2[new_idx] == NULL) {
                    table2[new_idx] = new std::pair<K, V>(table[i]->first, table[i]->second); 
                    should_probe2[new_idx] = true;
                    break;
                }
            }
        }

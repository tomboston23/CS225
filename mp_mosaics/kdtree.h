...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode * build(vector<Point<Dim>> &points, int leftIndex, int rightIndex, int depth);
    void clear(KDTreeNode* node);
    Point<Dim> findNearesthelper(const Point<Dim>& query, int dim, KDTreeNode* currRoot) const;
    double distance(Point<Dim> a, Point<Dim> b) const;

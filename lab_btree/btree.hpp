...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    //make sure first_larger_idx is outside the vector's range before checking
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->is_leaf){
        ...
        BTreeNode * leNut = subroot->children[first_larger_idx];
        insert(leNut, pair);
        if(leNut->elements.size() > order - 1){ //check if there's too many elements

                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(mergeQueue.empty()){
        TreeNode * singler = singleQueue.front();
        singleQueue.pop();
        return singler;
    }
    if(singleQueue.empty()){
        TreeNode * merger =  mergeQueue.front();
        mergeQueue.pop();
        return merger;
    }
    TreeNode* single = singleQueue.front();
    TreeNode* merge = mergeQueue.front();
    if(single->freq < merge->freq){ //return single and pop
        singleQueue.pop();
        return single;
    } else {
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    //first step: sort frequencies into singleQueue
    ...
    while(!((singleQueue.empty() && mergeQueue.size() == 1) || (singleQueue.size() == 1 && mergeQueue.empty()))){
        TreeNode * rem_1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode * rem_2 = removeSmallest(singleQueue, mergeQueue);   
        int total_freq = rem_1->freq.getFrequency() + rem_2->freq.getFrequency();
        TreeNode * parent = new TreeNode(total_freq);
        parent->left = rem_1;
        parent->right = rem_2;
        mergeQueue.push(parent);
    }
    //now either singleQueue or mergeQueue is empty and one has one remaining node. This should be the root
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
    ...
    //from here on we assume it's not a leaf node
    bfile.writeBit(0);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit()){//leaf
        TreeNode * node = new TreeNode(Frequency(bfile.getNextByte(), 0));
        return node;
    } else {
        TreeNode * chode = new TreeNode(Frequency(0));
        chode->left = readTree(bfile);
        chode->right = readTree(bfile);
        return chode;
    }
    return NULL;
}

...
template <typename T>
bool BinaryTree<T>::isOrderedRecursive(Node* currentNode, Node*& prevNode) const {
    if (currentNode == NULL) {
        return true;
    }
    if (currentNode != NULL){
        if (isOrderedRecursive(currentNode->left, prevNode) == false){
            return false;
        }
        if (prevNode != NULL && prevNode->elem > currentNode->elem){
            return false;
        }
        prevNode = currentNode;
        return isOrderedRecursive(currentNode->right, prevNode);
    }
    return true;
}